...
export default function Restaurant({api , handleRowClick, restaurant = false}) {
  ...
  return (
    <>
      {(api?.shuttleList?.length > 0 || api?.restaurantList?.length > 0)? (
        ...
        vụ Ẩm thực & Đồ uống chuyên nghiệp, đặc sắc, mang đến những
         trải nghiệm độc đáo và riêng biệt cho khách hàng."
          ...
          <Slide ref={slideRef} indicators={true} autoplay={false} canSwipe={false}>
            {(api?.shuttleList || api?.restaurantList).map((slide, index) => (
                    ...
                    <Row gutter={[24, 24]}
                      ...
                      <Col xs={24} md={9}>
                        ...
                        <div className="flex items-center text-gray-600 ">
                          ...
                          <div className="text-sm text-white">
                            ...
                            {!restaurant && (
                              <div
                                className="mb-1">{(slide.carPrice?.toLocaleString() || slide?.restaurantAdultPrice)}/VNĐ
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <Col xs={24} md={15}>
                        <p className="text-sm leading-relaxed text-white">
                          {(slide.carDescription || slide?.restaurantDescription)}
                        </p>
                      </Col>
                    </Row>
                    <Button
