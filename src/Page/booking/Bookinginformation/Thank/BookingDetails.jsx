    ...
    <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-xl p-8 mb-8 max-w-2xl mx-auto transform hover:scale-[1.01] transition-all duration-300">
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center pb-4 border-b border-gray-200">
          ...
          <span className="font-semibold text-navy">{listIDbook?.roomName}</span>
        </div>

        <div className="flex justify-between items-center pb-4 border-b border-gray-200">
          <span className="text-gray-600">Ngày nhận phòng</span>
          <span className="font-semibold">{`${listIDbook?.checkInDate[2]}-${listIDbook?.checkInDate[1]}-${listIDbook?.checkInDate[0]}`}</span>
        </div>

        <div className="flex justify-between items-center pb-4 border-b border-gray-200">
          <span className="text-gray-600">Ngày trả phòng</span>
          <span className="font-semibold">{`${listIDbook?.checkOutDate[2]}-${listIDbook?.checkOutDate[1]}-${listIDbook?.checkOutDate[0]}`}</span>
        </div>
